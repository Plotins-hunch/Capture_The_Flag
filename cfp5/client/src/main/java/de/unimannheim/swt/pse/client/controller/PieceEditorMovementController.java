...
public class PieceEditorMovementController implements Initializable {
  /**
  *label to display error messages if the input can't be turned into a piece
  ...
  /**
   * text field to enter the power of the piece
   */
  @FXML
  ...
  /**
   * radiobutton to select the special movement
   */
  @FXML
  private RadioButton specialMovement;
  /**
   * textfield to enter the up movement
   */
  @FXML
  private TextField up;
  /**
   * textfield to enter the down movement
   */
  @FXML
  private TextField down;
  /**
   * textfield to enter the left movement
   */
  @FXML
  private TextField left;
  /**
   * textfield to enter the right movement
   */
  @FXML
  private TextField right;
  /**
   * textfield to enter the upLeft movement
   */
  @FXML
  private TextField upLeft;
  /**
   * textfield to enter the upRight movement
   */
  @FXML
  private TextField upRight;
  /** background Pane*/
  @FXML
  GridPane background;
  /**
   * textfield to enter the downLeft movement
   */
  @FXML
  private TextField downLeft;
  /**
   * textfield to enter the downRight movement
   */
  @FXML
  private TextField downRight;
  /** Scene of the Map Editor*/
  ...
  /**
   * @author paklaus
  initalizes the PieceEditorMovement Scene
   */
  @Override
  public void initialize(URL arg0, ResourceBundle arg1){
    specialMovement.heightProperty().addListener((observable, oldValue, newValue) -> {
  ...
  public void addPiece(){
    ...
    int totalWidth = (int) mapEditorController.getPlayingFieldBackground().getWidth();
    int totalHeight = (int) mapEditorController.getPlayingFieldBackground().getHeight();
    HelperMethods.showMap(mapEditorController.getPlayingFieldArr(), mapEditorController.getPlayingField(), mapEditorController.getPieceDescriptions(), totalWidth, totalHeight, false, mapEditorController.getTheme());
    closeWindow();
  }
  ...
  public PieceDescription createPiece(){
    ...
    Movement movement = new Movement();
      movement.setDirections(getDirections());
    if(movement.getDirections() == null){
      return null;
    }
   ...
   */
  public void switchToPieceEditorLShape() {
    FXMLLoader fxmlLoader = new FXMLLoader();
    fxmlLoader.setLocation(getClass().getResource("/fxml/PieceEditorLShape.fxml"));
    Parent root = null;
    try {
    ...
    PieceEditorLShapeController controller = fxmlLoader.getController();
    ...
    Stage stage = (Stage) specialMovement.getScene().getWindow();
    stage.close();
  }
   ...
   * returns the directions object created from the input of the GUI
   */
  private Directions getDirections() {
    Directions directions = new Directions();
    if(!up.getText().isEmpty()){
      directions.setUp(Integer.parseInt(up.getText()));
    }
    if(!down.getText().isEmpty()){
      directions.setDown(Integer.parseInt(down.getText()));
    }
    if(!left.getText().isEmpty()){
      directions.setLeft(Integer.parseInt(left.getText()));
    }
    if(!right.getText().isEmpty()){
      directions.setRight(Integer.parseInt(right.getText()));
    }
    if(!upLeft.getText().isEmpty()){
      directions.setUpLeft(Integer.parseInt(upLeft.getText()));
    }
    if(!upRight.getText().isEmpty()){
      directions.setUpRight(Integer.parseInt(upRight.getText()));
    }
    if(!downLeft.getText().isEmpty()){
      directions.setDownLeft(Integer.parseInt(downLeft.getText()));
    }
    if(!downRight.getText().isEmpty()){
      directions.setDownRight(Integer.parseInt(downRight.getText()));
    }
    if(directions.getUp() == 0 && directions.getDown() == 0 && directions.getLeft() == 0 && directions.getRight() == 0 &&
        directions.getUpLeft() == 0 && directions.getUpRight() == 0 && directions.getDownLeft() == 0 && directions.getDownRight() == 0){
      errorMessage.setText("Please enter at least one direction");
      return null;
    }
    return directions;
  }
